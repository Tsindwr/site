...
The Dexterity Ability allows a character to use their Finesse Potential instead of their Might Potential when making Tests with melee weapons. This does not apply to any weapon with the [[equipment|Heavy]] or [[equipment|Unwieldy]] properties. *This ability can be assigned a prerequisite of any of the following archetypes, reducing its cost by 2 Strings:* Saboteur, Frontliner, or Summoner.
...
An Ability's reset condition should by default be the same for each side of the Action Card, otherwise the player must pay the difference in their Flipside. If there is a difference in an Action Card's reset condition is different on either side, whichever reset period is longer takes precedence. Meaning, if you have a Short Rest Attack and a General Movement you can use the Movement side of the Action Card as much as you like, until you use the Attack side, in which case you need to wait until a Short Rest until you can use either side of the Action Card.
...
| Initial\* | 1       | 0            |
| +1dX      | +1      | 1            |
\* The initial amount of damage will always be the Volatility Die that corresponds to the Test Skill's Potential. If it is an AOE effect, it associates with either the Spellcasting Potential if it is a spell, or the primary Potential chosen when acquiring the Archetype this Ability is for. 
...
Thread-costing modifiers usually include:
- +1 additional target
- any AOE
